...
# 相關變數(可以調整)
layer           = 5                                 # 2 < layer < 7
vicChessNum     = 2000                              # 參考的勝利棋譜數量
failChessNum    = 2000                              # 參考的失敗棋譜數量
...
for i in range(0, totalCircle):
    ...
    # 電腦決定刪除圈圈號碼
    if  i % 2 == 0  :  deleteNum, readyToDeleteSet  = game_fuctions.algo2_Round(choiceListNow, vicChessList, failChessList, deleteOrder)
    ...
    # 抽出失敗棋譜
    failChessList = game_fuctions.failChessExtract(failChessList, deleteOrder)
